...
export class WorldsFormat {
  ...
  constructor({ matchups, matchScores }: WorldsInitializer) {
    ...
    this.groups = [0, 1].map(i =>
      new WorldsGroup(matchups.slice(i * 4, i * 4 + 4), i, this)
    ) as [WorldsGroup, WorldsGroup];
    const matches = [0, 1].flatMap(i =>
      [
...
export class WorldsGroup {
  ...
  constructor(matchups: PartialMatchup[], index: number, format: WorldsFormat) {
    const lowerPlayoffs = format.playoffs.quarters.slice((1 - index) * 2, (1 - index) * 2 + 2);
    const upperPlayoffs = format.playoffs.quarters.slice(index * 2, index * 2 + 2);
    this.lowerSemis = [0, 1].map(i => new BracketNode(4, lowerPlayoffs[1 - i].slots[1]));
    this.lowerQuarters = [0, 1].map(i => new BracketNode(4, this.lowerSemis[i].slots[1]));
    this.upperSemis = [0, 1].map(i =>
      new BracketNode(4, upperPlayoffs[i].slots[0], this.lowerSemis[1 - i].slots[0])
    );
    this.upperQuarters = [0, 1].flatMap(i => [0, 1].map(j =>
      new BracketNode(4, this.upperSemis[i].slots[j], this.lowerQuarters[i].slots[j], matchups[i * 2 + j])
    ));
  }
}
...
export class WorldsPlayoffsFormat {
  ...
  constructor() {
    ...
    this.quarters = [0, 1].flatMap(i => [0, 1]
      .map(j => new BracketNode(4, this.semis[i].slots[j])));
  }
}