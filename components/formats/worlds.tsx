...
import { WorldsFormat, WorldsGroup, WorldsPlayoffsFormat } from "libs/formats/worlds";
...
function WorldsGroups({
  ...
  return (<div>
    <div className={styles.groups} ref={formatRef}>
      <Group group={groups[0]} nextRef={groupBRef} redrawFormat={redrawFormat} />
...
const Group = forwardRef(({
  ...
  return (
    <div className={doubleElimStyles['double-elim']}>
      <DoubleElimColumn upper={group.upperQuarters} lower={group.lowerQuarters} upperTitle='UB Quarterfinals'
        lowerTitle='LB Quarterfinals' ref={ref} {...formatProps} nextRef={semisRef} />
      <DoubleElimBracketLinesColumn upperCount={2} lowerCount={2} lowerIsStraight />
      <DoubleElimColumn upper={group.upperSemis} lower={group.lowerSemis} upperTitle='UB Semifinals'
        lowerTitle='LB Semifinals' ref={semisRef} {...formatProps} />
    </div>
  );
});
...
function WorldsPlayoffs({
  ...
  return (
    <div className={styles.playoffs}>
      <Column matches={bracket.quarters} title={"Quarterfinals"} redrawFormat={redrawFormat} />
      <BracketLinesColumn count={2} />
      <Column matches={bracket.semis} title={"Semifinals"} redrawFormat={redrawFormat} />
      <BracketLinesColumn count={1} />
      <Column matches={[bracket.final]} title={"Grand Final"} redrawFormat={redrawFormat} />
    </div>
  );
}