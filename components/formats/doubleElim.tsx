...
export default function DoubleElim({
  ...
  return (<div>
    <div className={styles['double-elim']} ref={bracketRef}>
      <DoubleElimColumn upper={bracket.upperR1} lower={bracket.lowerR1} upperTitle='UB Round 1'
        lowerTitle='LB Round 1' ref={columnRefs[0]} nextRef={columnRefs[1]} redrawFormat={redrawFormat} />
      <DoubleElimBracketLinesColumn upperCount={4} lowerCount={4} lowerIsStraight />
      <DoubleElimColumn upper={bracket.upperQuarters} lower={bracket.lowerR2} upperTitle='UB Quarterfinals'
        lowerTitle='LB Round 2' ref={columnRefs[1]} nextRef={columnRefs[2]} redrawFormat={redrawFormat} />
      <DoubleElimBracketLinesColumn upperCount={4} lowerCount={2} upperIsStraight />
      <DoubleElimColumn upper={2} lower={bracket.lowerR3} lowerTitle='LB Round 3' ref={columnRefs[2]}
        nextRef={columnRefs[3]} redrawFormat={redrawFormat} />
      <DoubleElimBracketLinesColumn upperCount={2} lowerCount={2} upperIsStraight lowerIsStraight />
      <DoubleElimColumn upper={bracket.upperSemis} lower={bracket.lowerQuarters} upperTitle='UB Semifinals'
        lowerTitle='LB Quarterfinals' ref={columnRefs[3]} nextRef={columnRefs[4]} redrawFormat={redrawFormat} />
      <DoubleElimBracketLinesColumn upperCount={2} lowerCount={1} upperIsStraight />
      <DoubleElimColumn upper={1} lower={[bracket.lowerSemi]} lowerTitle='LB Semifinal'
        ref={columnRefs[4]} nextRef={columnRefs[5]} redrawFormat={redrawFormat} />
      <DoubleElimBracketLinesColumn upperCount={1} lowerCount={1} upperIsStraight lowerIsStraight />
      <DoubleElimColumn upper={[bracket.upperFinal]} lower={[bracket.lowerFinal]} upperTitle='UB Final'
        ref={columnRefs[5]} nextRef={columnRefs[6]} lowerTitle='LB Final' redrawFormat={redrawFormat} />
      ...
      <div className={styles['grand-final-column']}>
        ...
        <div className={styles['grand-final-match']}>
          <Match node={bracket.grandFinal} ref={columnRefs[6]} redrawFormat={redrawFormat} />
        </div>
      </div>
    </div>
...
const DoubleElimColumn = forwardRef(({
  ...
  return (
    <div className={styles['double-elim-column']}>
      <DoubleElimInnerColumn matches={upper} title={upperTitle} {...upperProps} />
      {/* Gutter between upper and lower bracket */}
      <div></div>
      <DoubleElimInnerColumn matches={lower} title={lowerTitle} {...lowerProps} />
    </div>
  );
});

const DoubleElimInnerColumn = forwardRef(({
  matches,
  title,
  ...formatProps
}: {
  ...
  return title ?
    <Column matches={matches as BracketNode[]} title={title} ref={ref} {...formatProps} />
    :
